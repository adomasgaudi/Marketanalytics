...
1. **`makeSectionsCollapsible()` (4834)** — the app **physically reparents DOM nodes from dashView into companiesView at runtime**. There is no clean per-page component boundary in the source; "Companies" literally _is_ dashboard sections moved after load. In React each of those sections must be re-authored as a component and placed on the correct route directly.
