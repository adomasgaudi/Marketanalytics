...
# claude.md Events Hooks Rules Context
...
Each rule = two rows: **description**, then _kind · type_. Sorted by theme, then TOP → MED → LOW → Unclear within each.
...
| Rule                                  | Description / mechanics                                                                                                                                                                                     |
| :------------------------------------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **UI-01** Screenshot-first design     | UI change → ask owner for a screenshot via AskUserQuestion, score it against the [UI.md](UI.md) rubric, then render → critique → refine (never one-shot); fix the space-wasting _class_, not the shown case |
...
- **Fabula = Fabula ir partneriai, UAB** — formerly _Viešųjų ryšių partneriai (VRP)_, code **124099127**, founded 1997-07-03; same entity, rebranded. Its `brand` = `Fabula`.
- Fabula gets special treatment: own section at top, gold highlight in charts, pinned explorer row.

## Files & build

Folder layout (v51 reorg). Root keeps only files that _must_ live there:
