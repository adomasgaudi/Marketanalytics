...
(function () {
  ...
  var PAD = 'rgb(34 197 94 / 0.12)'; /* padding: green, like devtools — a tint, not a wall */
  ...
  function clearStamps() {
    ...
    stamped = [];
    if (boxStyle) { boxStyle.remove(); boxStyle = null; }
  }
  ...
  function stamp() {
    ...
    for (var i = 0; i < els.length; i++) {
      ...
      if (state.box && (s.display === 'flex' || s.display === 'inline-flex')) {
        el.dataset.dbg = 'flex';
        if (!el.title) el.title = s.display + ' · ' + s.flexDirection + ' · justify: ' + s.justifyContent + ' · align: ' + s.alignItems + ' · gap: ' + s.gap;
      } else if (state.box && s.display === 'grid') {
        el.dataset.dbg = 'grid';
        if (!el.title) el.title = 'grid · cols: ' + s.gridTemplateColumns + ' · gap: ' + s.gap;
      } else if (state.box && (s.position === 'absolute' || s.position === 'fixed' || s.position === 'sticky')) {
        el.dataset.dbg = 'pos';
        if (!el.title) el.title = 'position: ' + s.position + ' · t:' + s.top + ' r:' + s.right + ' b:' + s.bottom + ' l:' + s.left;
      }

      /* padding as inset green strips, margin as orange halo — per side, real widths */
      var shadows = [];
      if (state.pad) {
        var pt = px(s.paddingTop), pr = px(s.paddingRight), pb = px(s.paddingBottom), pl = px(s.paddingLeft);
      ...
      if (state.margin) {
        var mt = px(s.marginTop), mr = px(s.marginRight), mb = px(s.marginBottom), ml = px(s.marginLeft);
  ...
  function sides(s, prop) {
    return [s[prop + 'Top'], s[prop + 'Right'], s[prop + 'Bottom'], s[prop + 'Left']].map(px).join(' ');
  }
  function onMove(e) {
    var el = e.target;
    if (!(el instanceof HTMLElement) || el.closest('[data-devcorner]') || el === document.body) {
    ...
    tip.textContent =
      ...
      '\npad: ' + sides(s, 'padding') + '  margin: ' + sides(s, 'margin') +
      '\n' + s.fontSize + ' · ' + s.fontWeight + ' · ' + s.fontFamily.split(',')[0].replace(/"/g, '') +
      '\ncolor ' + toHex(s.color) + '  bg ' + toHex(s.backgroundColor);
    var x = e.clientX + 14, y = e.clientY + 14;
    if (x + 260 > innerWidth) x = e.clientX - 260;
    if (y + 80 > innerHeight) y = e.clientY - 90;
    tip.style.left = x + 'px';
    tip.style.top = y + 'px';
  }
  function applyInspect() {
    if (state.inspect) {
      if (!tip) {
        ...
        tip.style.cssText = 'position:fixed;z-index:99999;pointer-events:none;max-width:250px;white-space:pre-wrap;background:rgba(255,255,255,.95);border:1px solid #cbd5e1;border-radius:4px;padding:6px 8px;font:11px/1.5 monospace;color:#334155;box-shadow:0 2px 8px rgba(0,0,0,.15);display:none';
  ...
  function buildCorner() {
    ...
    corner.style.cssText = 'position:fixed;bottom:12px;left:12px;z-index:99998;display:flex;align-items:center;background:rgba(255,255,255,.92);border:1px solid #cbd5e1;border-radius:6px;font:11px monospace;color:#64748b;box-shadow:0 1px 4px rgba(0,0,0,.12);overflow:hidden';
    ...
    Object.keys(NOTE).forEach(function (k) {
      ...
      b.title = NOTE[k];
      b.type = 'button';
      b.style.cssText = 'border:none;background:none;font:inherit;padding:5px 7px;cursor:pointer;color:#64748b';
      b.addEventListener('click', function () {
        state[k] = !state[k];
        try { localStorage.setItem(KEY, JSON.stringify(state)); } catch (e) {}
        paint();
        if (k === 'inspect') applyInspect(); else stamp();
      });
      btns[k] = b;
    ...
    label.addEventListener('click', function () {
      collapsed = !collapsed;
      Object.keys(btns).forEach(function (k) { btns[k].style.display = collapsed ? 'none' : ''; });
    });
